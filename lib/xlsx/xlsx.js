...
function xlsx(file) {
	...
	var result, zip = new JSZip(), zipTime, processTime, s, f, i, j, k, l, t, w, sharedStrings, styles, external_links, rels_out, external_links_mapped, hyper_links, index, link_index, data, val, style, link, borders, border, borderIndex, fonts, font, fontIndex,
    ...
    fonts = new Array(1);
    	...
    	merges = [];
			...
			if (worksheet.table) {
				...
				rels_out = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/table" Target="../tables/table' + id + '.xml"/>'

				if (external_links.length >0){
						for (i = 0; i < external_links.length; i++) {
							rels_out += '<Relationship Id="rId'+(i+2)+'" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/hyperlink" Target="'+external_links[i]+'" TargetMode="External"/>';
						}
				}
