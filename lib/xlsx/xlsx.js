...
function xlsx(file) {
	...
	var result, zip = new JSZip(), zipTime, processTime, s, f, i, j, k, l, t, w, sharedStrings, styles, external_links, base_rels, rels_out, external_links_mapped, hyper_links, index, link_index, data, val, style, link, borders, border, borderIndex, fonts, font, fontIndex,
   ...
   if (typeof file === 'string') {
   	// Load
		...
		// Save
		processTime = Date.now();
		defaultFontName = file.fontName || defaultFontName;
		defaultFontSize = file.fontSize || defaultFontSize;
    ...
    fonts = new Array(1);
		...
		while (w--) {
			hyper_links = new Array();
			external_links = new Array();
			base_rels = 1;
			rels_out = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">';
    	...
    	merges = [];
            ...
            val = escapeXML(val.toString());
					...
					colWidth = cell.width || colWidth;
					// use stringified version as unic and reproductible style signature
					style = JSON.stringify(style);

					if(cell.link){
						link_index = external_links.indexOf(cell.link);
						if (link_index < 0) {
							link = external_links.push(cell.link) - 1;
						} else {
							link = link_index;
						}
						hyper_links.push('<hyperlink display="" location="" r:id="rId'+(link+base_rels)+'" ref="'+numAlpha(j) + (i + 1)+'" tooltip=""/>');
					}
			...
			for (i = 0; i < columns.length; i++) {
				if (columns[i].autoWidth) {
					cols.push('<col min="', i+1, '" max="', (i+1) * 1.2, '" width="', columns[i].max * 1.2, '" bestFit="1"/>');
				}
			}
			...
			if (hyper_links.length > 0) {
				s += '<hyperlinks>';
				for (i = 0; i < hyper_links.length; i++) {
					s += hyper_links[i];
				}
				s += '</hyperlinks>';
			}
			...
			if (worksheet.table) {
				...
				rels_out += '<Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/table" Target="../tables/table' + id + '.xml"/>';
			...
			if (external_links.length >0){
					for (i = 0; i < external_links.length; i++) {
						rels_out += '<Relationship Id="rId'+(i+base_rels)+'" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/hyperlink" Target="'+external_links[i]+'" TargetMode="External"/>';
					}
			}

			if(external_links.length > 0 || worksheet.table){
				rels_out += '</Relationships>';
				xlWorksheets.folder('_rels').file('sheet' + id + '.xml.rels', rels_out);
			}
