...
function xlsx(file) {
	...
	var result, zip = new JSZip(), zipTime, processTime, s, f, i, j, k, l, t, w, sharedStrings, styles, external_links, external_links_mapped, hyper_links, index, link_index, data, val, style, link, borders, border, borderIndex, fonts, font, fontIndex,
   ...
   if (typeof file === 'string') {
    ...
    fonts = new Array(1);
		...
		while (w--) {
			hyper_links = new Array(1);
			external_links = new Array(1);
    	...
    	merges = [];
            ...
            val = escapeXML(val);
					...
					if(cell.link){
						...
						hyper_links.push('<hyperlink display="" location="" r:id="rId'+(link+2)+'" ref="'+numAlpha(j) + (i + 1)+'" tooltip=""/>'{
							link_index:link,
						});
					}
			...
			if (worksheet.table) {
				...
				var rels_out = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/table" Target="../tables/table' + id + '.xml"/>'

				if (external_links.length >0){
						for (i = 0; i < external_links.length; i++) {
							rels_out += '<Relationship Id="rId'+(2+i)+'" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/hyperlink" Target="'+external_links[i]+'" TargetMode="External"/>';
						}
				}
